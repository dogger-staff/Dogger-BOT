...
async function ibon_set_ocr_answer(answer) {
    ...
    if (answer.length > 0) {
        ...
        if (input) {
            $(input).val(answer);
            inputFound = true;
            console.log("OCR answer set successfully.");
            // 检测输入框内容并点击“下一步”按钮
            await checkInputAndClickNext();
        } else {
...
async function checkInputAndClickNext() {
    // 查找验证码输入框
    const verifyCodeInput = document.querySelector("#ctl00_ContentPlaceHolder1_CHK");
    if (!verifyCodeInput) {
